...
for image in tqdm(images_with_ship):
    ...
    for prop in props:
        bboxes.append([train_image_dir + '/' + image, prop.bbox[1], prop.bbox[0], prop.bbox[4], prop.bbox[3], 'ship'])
...
print(bboxes_df.head().to_string(index=False))

bboxes.to_csv('train_bboxes.csv', index=False)
